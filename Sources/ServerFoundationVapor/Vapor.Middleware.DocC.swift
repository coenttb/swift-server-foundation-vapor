...
extension Vapor.Middlewares {
  public struct DocC: AsyncMiddleware {
    ...
    public init(
      archivePath: URL,
      redirectRoot: String? = nil,
      redirectMissingTrailingSlash: Bool = false
    ) {
      ...
      async throws -> Vapor.Response
    {
      ...
      if path == "data/documentation.json" {
        if FileManager.default.fileExists(
          atPath: archivePath.appendingPathComponent("data/documentation.json", isDirectory: true)
            .path
        ) {
          return try await serveStaticFileRelativeToArchive(
        ...
        request.logger.info(
          "\(self.prefix)data/documentation.json was not found, attempting to find product's JSON in /data/documentation/ directory"
        )

        let documentationPath = archivePath.appendingPathComponent(
          "data/documentation",
          isDirectory: true
        )
        do {
          ...
          return try await serveStaticFileRelativeToArchive(
            "data/documentation/\(productJSON)",
            request: request
          )
        } catch {
          return try await next.respond(to: request)
    ...
    private func serveStaticFileRelativeToArchive(_ staticFilePath: String, request: Request)
